...
public class DataExtractShadows : MonoBehaviour
{
    // Record
    ...
    // Tracking For Pose
    public GameObject TrackingBody;
    private Transform[] TrackingList;

    // ExcelFile
    private int fileNum;
    public int trial;
    public string excelFolderName = "/Data";
    public string FileName = "exp_data";
    ...
    void Start()
    {
        // Assign trackingList value
        var listLength = TrackingBody.transform.childCount;
        TrackingList = TrackingBody.GetComponentsInChildren<Transform>();
        ...
        FileName = excelFolderName + "/" + excelFileName + fileNum + ".xlsx";
        InitializeExcelSheet(FileName, excelSheetName + trial.ToString());
    ...
    void UpdateData()
    {
        if (isRecording)
        {
            ...
            {
                var tf = TrackingList[i].transform;
    ...
    public void InitializeExcelSheet(string excelFileName, string sheetName)
    {
        ...
        for (int i = 0; i < TrackingList.Length; i++)
        {
            var trackerName = TrackingList[i].name;
